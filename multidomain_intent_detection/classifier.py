...
from multidomain_intent_detection.normalizer import normalize_query, extract_entities, sanitize_for_embedding
from config.config import settings as _app_settings
from multidomain_intent_detection.llm_entity_extractor import extract_drug_pharmacy_entities
...
# GCS location for the model when it is absent locally (>150 MB, not in git)
_GCS_MODEL_URI = os.environ.get(
    "INTENT_MODEL_GCS_URI",
    "gs://vertex-gcs-pbm-nonprod-myclaims/Intent-Detection/v3_pipeline.pkl",
)
# Cached path after a GCS download so we only pull once per process
_gcs_download_path: Optional[str] = None
_gcs_download_lock = threading.Lock()
...
def _download_model_from_gcs(gcs_uri: str) -> str:
    """Download the model pickle from GCS to a temp file; return the local path.

    The temp file persists for the lifetime of the process (delete=False).
    A module-level lock ensures only one thread performs the download.
    """
    global _gcs_download_path
    with _gcs_download_lock:
        if _gcs_download_path and os.path.isfile(_gcs_download_path):
            return _gcs_download_path
        ...
        try:
            ...
            raise ImportError(
                "google-cloud-storage is required to load the model from GCS. "
                "Run: pip install google-cloud-storage"
            ) from exc

        logger.info(
            "[INTENT-MODEL] ========================================\n"
            "[INTENT-MODEL]  Downloading model from GCS\n"
            f"[INTENT-MODEL]  URI : {gcs_uri}\n"
            "[INTENT-MODEL] ========================================"
        )
        t0 = time.time()
        client = gcs.Client()
        ...
        blob.reload()
        remote_size_mb = (blob.size or 0) / (1024 * 1024)
        ...
        local_size_mb = os.path.getsize(tmp.name) / (1024 * 1024)
        logger.info(
            "[INTENT-MODEL] ========================================\n"
            "[INTENT-MODEL]  GCS download complete\n"
            f"[INTENT-MODEL]  Temp path  : {tmp.name}\n"
            f"[INTENT-MODEL]  Size       : {local_size_mb:.1f} MB  (remote: {remote_size_mb:.1f} MB)\n"
            f"[INTENT-MODEL]  Duration   : {elapsed:.1f}s\n"
            "[INTENT-MODEL] ========================================"
        )
        _gcs_download_path = tmp.name
        return _gcs_download_path
...
class MultidomainIntentClassifier:
    ...
    def _ensure_loaded(self):
        ...
        with self._load_lock:
            ...
            if os.path.exists(self._model_path):
                ...
                logger.info(
                    "[INTENT-MODEL] ========================================\n"
                    "[INTENT-MODEL]  Loading model from LOCAL DISK\n"
                    f"[INTENT-MODEL]  Path : {self._model_path}\n"
                    f"[INTENT-MODEL]  Size : {size_mb:.1f} MB\n"
                    "[INTENT-MODEL] ========================================"
                )
            else:
                logger.warning(
                    "[INTENT-MODEL] ========================================\n"
                    "[INTENT-MODEL]  Local model not found — falling back to GCS\n"
                    f"[INTENT-MODEL]  Tried : {self._model_path}\n"
                    f"[INTENT-MODEL]  GCS   : {_GCS_MODEL_URI}\n"
                    "[INTENT-MODEL] ========================================"
                )
                self._model_path = _download_model_from_gcs(_GCS_MODEL_URI)
            ...
            source_tag = "GCS" if self._model_path == _gcs_download_path else "LOCAL"
            logger.info(
                "[INTENT-MODEL] ========================================\n"
                "[INTENT-MODEL]  Multidomain classifier READY\n"
                f"[INTENT-MODEL]  Source     : {source_tag}  ({self._model_path})\n"
                f"[INTENT-MODEL]  Intents    : {n_intents}\n"
                f"[INTENT-MODEL]  PCA dims   : {self._pipeline.n_pca}\n"
                f"[INTENT-MODEL]  Load time  : {self._load_time * 1000:.0f} ms\n"
                "[INTENT-MODEL] ========================================"
            )
    ...
    def classify(self, query: str) -> Dict[str, Any]:
        ...
        # 1. Extract structured entities (claim number, NDC, NPI, member ID, dates)
        entities = extract_entities(query)

        # 2. Extract drug/pharmacy names via LLM — regex cannot handle open-ended
        #    product names like "DEXCOM G7 SENSOR" or "Metformin 500mg".
        #    Returns {} on failure so the pipeline degrades gracefully.
        drug_pharmacy = extract_drug_pharmacy_entities(query)
        entities.update(drug_pharmacy)

        # 3. Replace drug/pharmacy names with generic placeholders so the embedding
        #    focuses on intent semantics, not the specific entity text.
        #    The original values in `entities` flow through unchanged to the API call.
        sanitized = sanitize_for_embedding(query, entities)

        # 4. Normalize query (strip claim/sequence numbers) for embedding
        normalized = normalize_query(sanitized) if _app_settings.is_normalised else sanitized
        ...
        llm_confidence_val: Optional[float] = None
        if confident or not self.use_llm_fallback:
            ...
            # llm_classify with return_details=True returns a dict;
            # without the flag it returns a bare intent string (legacy callers).
            if isinstance(llm_result, dict):
                final_intent = llm_result.get("intent") or pred["intent"]
                llm_thinking = llm_result.get("thinking")
                llm_reasoning = llm_result.get("reasoning")
                llm_confidence_val = llm_result.get("confidence")
            else:
