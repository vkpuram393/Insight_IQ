...
def search_pca(X, y, labels) -> int:
    ...
    pca_range = [50, 75, 100, 120, 140, 150, 175, 200, 250, 300, 400, 500]
    fixed_dims = None
    min_variance = 0.85  # prefer dims that retain at least this fraction of variance
    if os.path.exists(cfg_path):
        try:
            ...
            min_variance = pipe_cfg.get("pca_min_variance", min_variance)
    ...
    print(f"  Minimum variance retention: {min_variance * 100:.0f}%")
    print(f"{'=' * 70}")
    ...
    for d in pca_range:
        if d >= X.shape[0]:
            continue
        var_pct = _PCA(n_components=d, whiten=True, random_state=42).fit(X_norm).explained_variance_ratio_.sum()
        p = IntentPipeline(n_pca=d)
        p.label_names = labels
        a, s, _ = p.cross_validate(X, y)
        results.append((d, var_pct, a, s))
        star = ""
        # Prefer higher dims if accuracy is within 0.2% AND variance retention
        # is significantly better (meets min_variance threshold while current
        # best does not, or both meet it but acc is truly higher).
        is_better = False
        if a > best_a + 1e-6:
            is_better = True
        elif abs(a - best_a) <= 0.002 and var_pct >= min_variance and best_var < min_variance:
            # Tie-break: if accuracy is essentially equal (<0.2% diff), prefer
            # the candidate that meets the minimum variance floor
            is_better = True
        elif abs(a - best_a) <= 0.002 and var_pct > best_var + 0.02:
            # Tie-break: same accuracy within 0.2%, pick higher variance
            is_better = True
        ...
        if is_better:
            ...
            best_d = d
            best_var = var_pct
        print(f"  {d:>8} {var_pct * 100:>7.1f}% {a * 100:>8.2f}% {s * 100:>6.2f}%{star}")

    # Warn if the selected dims don't meet the variance floor
    if best_var < min_variance:
        print(f"\n  ⚠ WARNING: Best PCA-{best_d} retains only {best_var * 100:.1f}% variance")
        print(f"    (below {min_variance * 100:.0f}% floor). Consider higher dims if override")
        print(f"    intents show low accuracy.")

    print(f"\n  Optimal: PCA-{best_d} ({best_a * 100:.2f}%, {best_var * 100:.1f}% variance)")
    print(f"{'=' * 70}\n")
    return best_d
