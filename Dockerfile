...
# Verify the model is present — fail the build clearly if CI forgot to download it
# Run: gsutil cp gs://vertex-gcs-pbm-nonprod-myclaims/Intent-Detection/v3_pipeline.pkl \
#           multidomain_intent_detection/artifacts/v3_pipeline.pkl
# before `docker build` so the file is included by the COPY above.
RUN test -f multidomain_intent_detection/artifacts/v3_pipeline.pkl \
    || (echo "ERROR: v3_pipeline.pkl not found — download it before building:" \
        && echo "  gsutil cp gs://vertex-gcs-pbm-nonprod-myclaims/Intent-Detection/v3_pipeline.pkl multidomain_intent_detection/artifacts/v3_pipeline.pkl" \
        && exit 1)
