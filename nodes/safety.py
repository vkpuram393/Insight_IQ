...
def _unmask_recommendations(
    recommendations: Optional[List[Dict[str, str]]],
    pii_service,
    combined_token_mapping: Optional[Dict[str, Any]] = None,
    session_id: str = ""
) -> Tuple[List[Dict[str, str]], int]:
    """
    Unmask PII/PHI tokens in recommendation chip texts.
    
    Recommendations are generated by the LLM using masked data, so they may contain
    tokens like [PERSON_A035EF42] or [MEMBER_ID_M078709008PCFEE]. This function
    restores original values using the same unmasking pipeline as the response text.
    
    Args:
        recommendations: List of recommendation dicts with 'text' and 'action' keys
        pii_service: PIIProtectionService instance for unmasking
        combined_token_mapping: Merged text + tool token mappings (may be None/empty)
        session_id: Session ID for token_storage fallback lookup
        
    Returns:
        Tuple of (unmasked_recommendations, tokens_unmasked_count)
    """
    if not recommendations:
        return recommendations or [], 0
    ...
    for rec in recommendations:
        if not isinstance(rec, dict) or "text" not in rec:
            unmasked_recommendations.append(rec)
            continue
        
        original_text = rec["text"]
        unmasked_text = original_text
        
        # Step 1: Standard unmask using token mapping (same as response unmasking)
        if combined_token_mapping:
            unmasked_text = pii_service.unmask_pii_phi(unmasked_text, combined_token_mapping)
        
        # Step 2: Cleanup remaining tokens (handles hallucinated/orphan tokens)
        unmasked_text = pii_service.cleanup_remaining_tokens(
            unmasked_text, combined_token_mapping
        )
        
        # Step 3: Token storage fallback (handles cross-turn tokens not in mapping)
        if unmasked_text != original_text:
            tokens_unmasked += 1
            logger.info(f"   🔓 Unmasked recommendation: '{original_text}' → '{unmasked_text}'")
        elif session_id and re.search(r'\[[A-Z_]+_[A-Fa-f0-9]{8}\]', unmasked_text):
            # Still has tokens — try token_storage fallback
            for token_match in re.finditer(r'\[([A-Z_]+_[A-Fa-f0-9]{8})\]', unmasked_text):
                full_token = f"[{token_match.group(1)}]"
                storage_key = f"{session_id}:{full_token}"
                if storage_key in pii_service.token_storage:
                    original_value = pii_service.token_storage[storage_key]
                    unmasked_text = unmasked_text.replace(full_token, str(original_value))
                    tokens_unmasked += 1
                    logger.info(f"   🔓 Unmasked recommendation token (storage fallback): '{full_token}' → '{original_value}'")
        
        unmasked_recommendations.append({**rec, "text": unmasked_text})
    
    if tokens_unmasked > 0:
        logger.info(f"✅ Unmasked {tokens_unmasked} token(s) in {len(unmasked_recommendations)} recommendations")
    
    return unmasked_recommendations, tokens_unmasked
...
async def response_safety_pii_postcheck_node(state: AgentState) -> Dict[str, Any]:
    """
    Unmask PII/PHI in the generated response, recommendations, and entities + Leakage detection
    ...
    3. Recommendation Unmasking: Restore original PII/PHI values in recommendation chip texts
    
    INPUT (from state):
        - response: LLM-generated response (with tokens)
        - recommendations: LLM-generated recommendation chips (may contain tokens)
        - session_id: For token lookup
        - metadata.response_pii_masking: Token mappings
    
    OUTPUT (to state):
        - response: UNMASKED response (real values restored)
        - recommendations: UNMASKED recommendations (real values restored)
    ...
    if not response:
        ...
        # Unmask recommendations (robustness: may be empty in LLM judge path)
        recommendations = state.get("recommendations", [])
        unmasked_recommendations, rec_tokens_unmasked = _unmask_recommendations(
            recommendations, pii_service, None, session_id
        )
        ...
        if rec_tokens_unmasked > 0:
            logger.info(f"✅ Unmasked {rec_tokens_unmasked} token(s) in recommendations (no-response path)")
        await log_state_snapshot(state, node_name, result)
        return result
    ...
    try:
        ...
        if state.get("error") or metadata.get("error_occurred"):
            ...
            # Unmask recommendations even on error path (best-effort)
            _text_tm = response_pii_masking.get("text_metadata", {}).get("token_mapping", {})
            _tool_tm = response_pii_masking.get("tool_metadata", {}).get("token_mapping", {})
            _combined_tm = {**_text_tm, **_tool_tm} if (_text_tm or _tool_tm) else None
            recommendations = state.get("recommendations", [])
            unmasked_recommendations, rec_tokens = _unmask_recommendations(
                recommendations, pii_service, _combined_tm, session_id
            )
            result = {
                "response": response,
                "recommendations": unmasked_recommendations,
                ...
                "metadata": {
                    **metadata,
                    ...
                    "recommendations_tokens_unmasked": rec_tokens
                }
            }
            await log_state_snapshot(state, node_name, result)
            return result
        ...
        if not response_pii_masking:
            ...
            # Unmask recommendations using token_storage fallback (no mapping available)
            recommendations = state.get("recommendations", [])
            unmasked_recommendations, rec_tokens = _unmask_recommendations(
                recommendations, pii_service, None, session_id
            )
        ...
        if leaked_entities:
            ...
            if leakage_mode == "block":
                ...
                result = {
                    ...
                    "recommendations": [],  # Clear recommendations when response is blocked
        ...
        if not combined_token_mapping:
            ...
            # Unmask recommendations using token_storage fallback (no mapping in this path)
            recommendations = state.get("recommendations", [])
            unmasked_recommendations, rec_tokens_unmasked = _unmask_recommendations(
                recommendations, pii_service, None, session_id
            )
                ...
                "metadata": {
                    **metadata,
                    ...
                    "response_pii_unmasking": {
                        ...
                        "recommendations_tokens_unmasked": rec_tokens_unmasked,
                        "fallback_path": True
                    }
                }
            }
        ...
        recommendations = state.get("recommendations", [])
        unmasked_recommendations, rec_tokens_unmasked = _unmask_recommendations(
            recommendations, pii_service, combined_token_mapping, session_id
        )
        ...
        result = {
            ...
            "entities": unmasked_entities if unmasked_entities else state.get("entities"),  # ← Unmask entities
            "recommendations": unmasked_recommendations,  # ← Unmask recommendation chip texts
            "safety_postcheck_passed": True,
            "metadata": {
                **metadata,
                "leakage_check": {"has_leakage": False},
        ...
        # Best-effort recommendation unmasking in error path
        try:
            _pii_svc = get_pii_service()
            _recs = state.get("recommendations", [])
            _unmasked_recs, _ = _unmask_recommendations(_recs, _pii_svc, None, session_id)
        except Exception:
            _unmasked_recs = state.get("recommendations", [])
        
        result = {
            "response": response,
            "recommendations": _unmasked_recs,
            "safety_postcheck_passed": True,
            "metadata": {
                **state.get("metadata", {}),
